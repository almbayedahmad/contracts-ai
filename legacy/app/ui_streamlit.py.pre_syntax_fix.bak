...
def _read_text(file):
    ...
    return data.decode("latin-1", errors="ignore")
            ...
            for t in doc.tables:
                for r in t.rows:
                    parts.extend([c.text.strip() for c in r.cells if c.text.strip()])
    try:
        log_event(_runlg, 'run_finish', run_id=run_id)
    except Exception:
        pass
    return "\n".join(parts)
            ...
            texts = [p.get_text("text") for p in pdf]
    try:
        log_event(_runlg, 'run_finish', run_id=run_id)
    except Exception:
        pass
...
try:
    ...
    results_dir.mkdir(parents=True, exist_ok=True)
    text, tables = _read_text_and_tables(file)
    ...
    try:
        ...
        pass
    return text, df_spans, comp_df, ents_df, links_df, sum_de, output
...
def _safe_run_pipeline(file):
    ...
    try:
        ...
        pass
    return run_local_pipeline(file)
